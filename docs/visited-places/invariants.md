...
- For bulk stores (`TryAddRange`, when `FetchedChunks.Count > 1`), **all** segments stored in the current event cycle are in the immune set — not just the last one. This prevents any of the newly-stored gap segments from being immediately re-evicted in the same event cycle.
...
- `engine.InitializeSegment(segment)` is called by `CacheNormalizationExecutor` immediately after each `_storage.TryAdd(segment)` returns `true`, or for bulk stores, for each segment in the array returned by `_storage.TryAddRange(segments[])`, which in turn calls `selector.InitializeMetadata(segment)`
