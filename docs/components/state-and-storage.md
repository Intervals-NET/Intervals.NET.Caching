...
**Single-Writer Rule (Invariant A.11):** Only `RebalanceExecutor` writes any field of `CacheState`. User path components are read-only. This is enforced by internal visibility modifiers (setters are `internal`), not by locks.
...
| A.12a     | `IsInitialized` only transitions false → true (monotonic)            |
| B.1       | Cache is always contiguous (no gaps in cached range)                 |
| B.2       | Cache updates are atomic via `Rematerialize`                         |
| B.3       | Consistency under cancellation: partial results discarded            |
| B.5       | Cache contiguity invariant maintained after every rematerialization  |
| E.5       | `NoRebalanceRange` is computed correctly from thresholds             |
| E.6       | `NoRebalanceRange` is always contained within `CacheRange`           |
